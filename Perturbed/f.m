...
global lambda_0 gamma_0 lambda_1 gamma_1 sigma randomsInterp randomsIndex
...
% Rho = noise ratio * a random number taken from the standard normal
% distribution:
%rho = sigma.*randn(1);
...
if (q == 0) 
    u = - lambda_0*y1 - gamma_0*(y2 + rho);
elseif (q == 1)
    u = - lambda_1*y1 - gamma_1*(y2 + rho); 
end
